...
// Loading FHE library
import {FHE, euint32, externalEuint32, ebool, euint64, externalEuint64} from "@fhevm/solidity/lib/FHE.sol";
...
contract EncryptedERC20 is Ownable2Step, SepoliaConfig {
    ...
    // address public constant allowList = []address(0);
    ...
    function addToAllowlist(address addr) external onlyOwner {
        require(!allowlist[addr], "Already allowlisted");
        allowlist[addr] = true;
        allowlistAddresses.push(addr);
        emit AddressAllowlisted(addr);
    }

    /// @notice Removes an address from the allowlist
    /// @param addr The address to remove
    function removeFromAllowlist(address addr) external onlyOwner {
        require(allowlist[addr], "Not allowlisted");
        allowlist[addr] = false;

        // Remove from array (optional: keeps array in sync for enumeration)
        for (uint i = 0; i < allowlistAddresses.length; i++) {
            if (allowlistAddresses[i] == addr) {
                allowlistAddresses[i] = allowlistAddresses[allowlistAddresses.length - 1];
    ...
    /// @dev Grant decryption permission to all allowlisted addresses for a given value
    function _allowForAllowlist(euint64 value) internal {
        for (uint i = 0; i < allowlistAddresses.length; i++) {
            address addr = allowlistAddresses[i];
            if (allowlist[addr]) {
